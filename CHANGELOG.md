...
- **斯大林格勒战役场景** — 苏军防御 vs 德军进攻的城市攻防战（伏尔加河、火车站争夺、雪暴→阴→晴天气脚本、双方增援）
- **回放与指标系统** — JSONL 回放加载器（离线重建推演、坏行容错）、Markdown 复盘报告（战况总览/指挥链健康度/目标控制时序/关键事件时间线）、导演部讲评面板新增战力/得分/目标控制三条 Canvas 曲线
- **AI 想定导入** — 从资料一键识别并生成动态战役场景

### Changed — 前端精致化与 LLM 路径优化
- **前端军事绿金风格全量重构 + 精致化打磨** — 简报电影级扫描线、地图大气暗角、面板深度、卡片金辉、hero 流光、讲评曲线
- **LLM 路径优化** — 指数退避重试、JSON 解析增强（剥离推理标签/截断修复/取末位合法对象）、三级 nudge 重试、trace 增加 policy_reason/llm_attempts/llm_latency_ms 字段
- **逐拍指标快照** — 引擎每 tick 记录兵力/存活/得分/目标控制，供讲评曲线绘制（带降采样）

### Fixed
- **复盘与在线讲评口径统一** — 复盘报告与导演部讲评共用同一数据管道，确认率/延迟统计一致
- **LLM 解析多候选歧义** — 模型先复述 schema 再给真实决策时，取最后一个合法 JSON

### Quality
- 单元测试扩充至 47 个（覆盖简报/场景/LLM 路径/回放/指标）
- **ruff 全量清零**（67 项告警：导入排序、注解去引号、死变量移除、嵌套 if 合并等）
