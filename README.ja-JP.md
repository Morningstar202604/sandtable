...
- **作戦ブリーフィングとアフターアクションレビュー**——各シナリオは開始前に表示される
  作戦ブリーフィングを同梱し、推演中は 5 ティックごとに指揮サマリー（天候/目標支配/兵力比）を
  出力します。戦闘後は Markdown のアフターアクションレポート（指揮系統の健全性、目標支配の
  時系列、主要イベント）をエクスポートできるほか、レビューパネルで戦力/スコア/目標支配の
  カーブをリアルタイムに確認できます。
